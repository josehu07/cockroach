...
func TestRSCodingFromIntoData(t *testing.T) {
	...
	assert.Nil(t, err)
	cw := codewordFromNull[testEntries](coder)
	...
	cw, err = codewordFromData(&input, coder)
	assert.Nil(t, err)
	...
	outputPtr, err := cw.convertIntoData(coder)
	assert.Nil(t, err)
	require.Equal(t, input, *outputPtr)

	cw.shards[1] = nil

	_, err = cw.convertIntoData(coder)
	assert.NotNil(t, err)
}
...
func TestRSCodingSubsetAbsorb(t *testing.T) {
	...
	coder, err := newCoder(3, 2)
	assert.Nil(t, err)
	cwa, err := codewordFromData[testEntries](&input, coder)
	assert.Nil(t, err)

	_, err = cwa.singleRef(5)
	assert.NotNil(t, err)

	_, err = cwa.subsetRefs([]bool{true, false, true, false, false, false})
	assert.NotNil(t, err)

	cw00, err := cwa.singleRef(2)
	assert.Nil(t, err)
	require.Equal(t, 3, cw00.numDataShards)
	require.Equal(t, 2, cw00.numParityShards)
	require.Equal(t, 1, cw00.availDataShards())
	require.Equal(t, 0, cw00.availParityShards())
	assert.NotNil(t, cw00.shards[2])
	...
	cwb := codewordFromNull[testEntries](coder)
